$(function () {
    ...
    document.onkeydown = function (e) {
        if (e.keyCode == 32)
            $('#next').click(); // space bar

        else if (e.keyCode == 27 && $('#edit').hasClass("firstClick") == true)
            $('#edit').click(); // escape key

        else if (e.keyCode == 13 && $('#edit').hasClass("secondClick") == true)
...
//        document.getElementById("next").disabled=true;
        ...
        if (status == true)
            exp = ",explicit";
        var url = "https://api.icndb.com/jokes/random?firstName=" + fn + "&lastName=" + ln + "&limitTo=[nerdy" + exp + "]&escape=javascript";
        fetch(url)
            .then(
                function (response) {
